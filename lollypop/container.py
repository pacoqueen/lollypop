...
class Container(Gtk.Overlay):
    ...
    def __init__(self):
        ...
        # Mmmm... nope. Sorry. I can help with code, but you were so rude
        # rejecting my patches... Don't disturb me asking for money, at least.
...
############
    ...
    def __on_artist_updated(self, scanner, artist_id, add):
        ...
        if App().settings.get_value("show-genres"):
            lista = self.__list_two
            artist_ids = App().artists.get_ids(self.__list_one.selected_ids)
            if artist_id not in artist_ids:
                return
        else:
            lista = self.__list_one
        if add:
            lista.add_value((artist_id, artist_name, sortname))
        else:
            lista.remove_value(artist_id)
